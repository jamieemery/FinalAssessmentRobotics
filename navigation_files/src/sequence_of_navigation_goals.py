...
class MoveBaseSeq():
    ...
    def additionalCleaningPoints(self, goalNumber, startPoint):
        ...
        for i in range(goalNumber):
            x_coord = input("X coord (5.8<=x<=25.2): ")
            y_coord = input("Y coord (8.5<=y<=19.6): ")
            newP_seq.append(x_coord)
            newP_seq.append(y_coord)
            newP_seq.append(0)
            spray = input("Spray amount (%): ")
            spray_needed.append(spray)
        num = 3
        newPoints = [newP_seq[i:i+num] for i in range(0, len(newP_seq), num)]
        yea_seq = goalNumber * [90]
        #List of goal quaternions:
        quat_seq = list()
        for yawangle in yea_seq:
            #Unpacking the quaternion tuple and passing it as arguments to Quaternion message constructor
        ...
        for point in newPoints:
            #Exploit n variable to cycle in quat_seq
            newSortedListOfPoints.append(Pose(Point(*point),quat_seq[num-3]))
            num += 1
        n = len(newSortedListOfPoints)
        for i in range(goalNumber):
            self.combine_pose_spray.append([newSortedListOfPoints[n-i-1], spray_needed[i]])
        notVisitedNodes = list()
        # transform points to Node objects
        for point in newSortedListOfPoints:
            notVisitedNodes.append(Node(point.position.x,point.position.y,point.position.z,point.orientation))
        startNode = Node(startPoint.position.x, startPoint.position.y, startPoint.position.z, startPoint.orientation)
        solution = TSP()
        sorted_nodes = solution.swapTSPAlgorithm(notVisitedNodes)
        # turn Node objects back to Pose objects for the Action server to use
